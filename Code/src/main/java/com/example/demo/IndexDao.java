...
public class IndexDao {
    private final static String SOLR_URL = "http://localhost:8984/solr/cyberminer";
    ...
    public List<KWICIndex> searchIndex(String queryString, int offset, int limit) {
        ...
        try {
            ...
            for (SolrDocument solrDoc : solrDocs) {
                ...
                kwicIndex.setUrl(String.valueOf(solrDoc.get("id")));
                kwicIndex.setDescription(String.valueOf(solrDoc.get("description")));

                foundIndices.add(kwicIndex);
            }
        } catch (Exception e) {
        }
